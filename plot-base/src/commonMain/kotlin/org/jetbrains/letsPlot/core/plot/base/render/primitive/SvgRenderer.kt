...
class SvgRenderer(private val root: SvgRoot, private val smooth: Boolean = false) : Renderer {
    ...
    private fun emitPath(points: List<DoubleVector>, stroke: StrokeStyle?, fill: FillStyle?, closed: Boolean, smooth: Boolean) {
    ...
    override fun drawRect(rect: DoubleRectangle, stroke: StrokeStyle?, fill: FillStyle?, radius: Double) {
        if (radius <= 0.0) {
            val el = SvgRectElement(rect)
            applyStyles(el, stroke, fill)
            root.add(el)
        } else {
            val el = SvgPathElement(roundedRectData(rect, radius).build())
            applyStyles(el, stroke, fill)
            root.add(el)
        }
    }
