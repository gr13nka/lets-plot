...
open class QuantilesHelper(
    ...
    // Each quantile line as a PolylineData, for the caller to render through the Renderer
    // (renderer.path).
    internal fun getQuantileLineSegments(
        ...
        val segments = mutableListOf<PolylineData>()
                ...
                while (currPointsIdx < sortedDataPoints.size) {
                    ...
                    if (quantile == p.quantile()) {
                        getQuantileLineGeometry(p, toLocationBoundStart, toLocationBoundEnd)
                            ?.let { segments.add(PolylineData(p, it)) }
                        break
                    }
                }
            }
    ...
    private fun getQuantileLineGeometry(
        ...
        val svgElementHelper = GeomHelper(pos, coord, ctx).createLineGeometryHelper()
