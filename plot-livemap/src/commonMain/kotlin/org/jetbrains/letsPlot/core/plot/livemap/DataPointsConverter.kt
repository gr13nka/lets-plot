    ...
    private inner class SinglePathFeatureConverter(
        ...
        fun curve(geom: CurveGeom): List<DataPointLiveMapAesthetics> {
            ...
            return process(isClosed = false) {
                ...
                // Raw unpadded geometry - livemap re-computes padding/arrows on each zoom level.
                GeomHelper.LineGeometryHelper()
                    .createCurveGeometry(start, end, geom.curvature, geom.angle, geom.ncp, it)
        ...
        fun spoke(geom: SpokeGeom): List<DataPointLiveMapAesthetics> {
            ...
            return process(isClosed = false) {
                ...
                GeomHelper.LineGeometryHelper()
                    .createSpokeGeometry(base, angle, radius, geom.pivot.factor, it)
                    ?: return@process emptyList()
            }
        }
