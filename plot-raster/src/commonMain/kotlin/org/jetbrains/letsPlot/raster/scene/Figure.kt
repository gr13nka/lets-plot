...
internal abstract class Figure : Node() {
    ...
    val fillPaint: Paint? by derivedAttr {
        return@derivedAttr fillPaint(fill, fillOpacity)
    }

    val strokePaint: Paint? by derivedAttr {
        if (strokeWidth < 0f) {
            return@derivedAttr null
        }

        return@derivedAttr strokePaint(
            stroke = stroke,
    ...
    companion object {
        val CLASS = ATTRIBUTE_REGISTRY.addClass(Figure::class)

        val StrokeAttrSpec = CLASS.registerVariableAttr(Figure::stroke)
        val StrokeWidthAttrSpec = CLASS.registerVariableAttr(Figure::strokeWidth, affectsBBox = true)
        val StrokeOpacityAttrSpec = CLASS.registerVariableAttr(Figure::strokeOpacity)
        ...
        val StrokePaintAttrSpec = CLASS.registerDerivedAttr(
            kProp = Figure::strokePaint,
            dependencies = setOf(
                StrokeAttrSpec,
                StrokeWidthAttrSpec,
        ...
        val FillPaintAttrSpec = CLASS.registerDerivedAttr(
            kProp = Figure::fillPaint,
            dependencies = setOf(FillAttrSpec, FillOpacityAttrSpec)
        )
